...
ggsave(filename=paste0(site_name,"/Verification-Minimum Temperature linear models.jpg"),plot=pLM,
       width=9, height=6, dpi=300)
```
...
ggsave(filename=paste0(site_name,"/Verification-Maximum Temperature linear models.jpg"),plot=pUH,
       width=9, height=6, dpi=300)
```
...
gfdl_low_reanal$rain_mm <- gfdl_low_raw$rain_mm + ifelse(diff_gfdl_low<0,0,diff_gfdl_low*10)
gfdl_high_reanal$rain_mm <- gfdl_high_raw$rain_mm + ifelse(diff_gfdl_high<0,0,diff_gfdl_high*10)
ukesm_low_reanal$rain_mm <- ukesm_low_raw$rain_mm + ifelse(diff_ukesm_low<0,0,diff_ukesm_low*10)
ukesm_high_reanal$rain_mm <- ukesm_high_raw$rain_mm + ifelse(diff_ukesm_high<0,0,diff_ukesm_high*10)

```

```{r verify_rain_plot_LM}
# calculate linear models
fit_hist <- coef(lm(h_rain_cm ~ year, 
                    data = hist_exp))
fit_baseln <- coef(lm(bl_rain_cm ~ year, 
...
ggsave(filename=paste0(site_name,"/Verification-Precipitation linear models.jpg"),plot=pVR,
       width=9, height=6, dpi=300)
...
## CMIPs
gfdl_low_raw_reanal <- read.csv(paste0("fut_clim_scenario_2_reanal.csv")) 
gfdl_low_raw_reanal <- read.csv("Data/CMIP6/LRF/fut_clim_scenario_2_reanal.csv") 
gfdl_low <- gfdl_low_raw %>%
  select(day,month,year,dayofyear,mint_C,maxt_C,rain_cm)
colnames(gfdl_low) <- c("day","month","year","dayofyear","gl_tmin","gl_tmax","gl_rain_cm")
gfdl_low$date=make_date(gfdl_low$year, gfdl_low$month, gfdl_low$day)

gfdl_high_raw_reanal <- read.csv(paste0(site_name,"/fut_clim_scenario_3_reanal.csv")) 
gfdl_high <- gfdl_high_raw %>%
  select(day,month,year,dayofyear,mint_C,maxt_C,rain_cm)
colnames(gfdl_high) <- c("day","month","year","dayofyear","gh_tmin","gh_tmax","gh_rain_cm")
gfdl_high$date=make_date(gfdl_high$year, gfdl_high$month, gfdl_high$day)

ukesm_low_raw_reanal <- read.csv(paste0(site_name,"/fut_clim_scenario_4_reanal.csv")) 
ukesm_low <- ukesm_low_raw %>%
  select(day,month,year,dayofyear,mint_C,maxt_C,rain_cm)
colnames(ukesm_low) <- c("day","month","year","dayofyear","ul_tmin","ul_tmax","ul_rain_cm")
ukesm_low$date=make_date(ukesm_low$year, ukesm_low$month, ukesm_low$day)

ukesm_high_raw_reanal <- read.csv(paste0(site_name,"/fut_clim_scenario_5_reanal.csv")) 
ukesm_high <- ukesm_high_raw %>%
  select(day,month,year,dayofyear,mint_C,maxt_C,rain_cm)
colnames(ukesm_high) <- c("day","month","year","dayofyear","uh_tmin","uh_tmax","uh_rain_cm")
ukesm_high$date=make_date(ukesm_high$year, ukesm_high$month, ukesm_high$day)
