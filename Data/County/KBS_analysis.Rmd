...
---
prerequisite: run 0_Observations_and_constants.R with arbitrary scenario
...
#calib_path <- paste0("Data/KBS/Calibration/")
#mgmt_path <- paste0("Data/KBS/Management/")
#wth_path <- paste0("Data/KBS/Weather/")
...
ggplot(aes(x=date, y=soil_temperature, color=treatment, show.legend=TRUE)) +
  geom_line() +
  geom_abline(intercept=ObsTfit_coef[1], slope=ObsTfit_coef[2], color="blue") +
...
###
ObsMfit <- lm(mean_VSM ~ year, data = SoilMoist_VSM)
...
gM <- SoilMoist_VSM %>%
ggplot(aes(x=year, y=mean_VSM, color=treatment, show.legend=TRUE)) +
  geom_point() +
  geom_abline(intercept=ObsMfit_coef[1], slope=ObsMfit_coef[2], color="blue") +
...
fert_df <- read.csv(file=paste0(calib_folder,"RFertilizer.csv"))
ops_ext_adj_raw <- read_csv(paste0(mgmt_folder,adjusted_ops_filename),
...
# rain
ObsPfit <- lm(rain_cm.x ~ date, data = historical_weather)
...
ggplot(aes(x=date, y=rain_cm.x, color=treatment, show.legend=TRUE)) +
  geom_point() +
...
ObsPfit2 <- lm(rain_cm.x ~ year, data = historical_weather)
