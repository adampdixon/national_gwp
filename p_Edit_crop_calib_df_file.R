...
p_Edit_crop_calib_df_file <- function(data_mtx) {
  
  print("Starting p_Edit_crop_calib_df_file")
  
  outfile_name <- paste0(results_path,"calib_crop_df.csv")
  ...
  write.table(data_mtx,file=paste0(outfile_name),
              col.names=c("year","Observed","Obs_sd","APSIM","Daycent","crop",
                          "treatment_scen"),
              row.names=F,sep=",")
