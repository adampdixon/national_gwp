...
summary_Climate <- function(climvariable="prcp",year=1951){
 ...
 year_results <-  foreach(n = 1:12, .packages=c("tidyverse","terra","data.table"), .combine = rbind) %do% {
    ...
    allday <- foreach(dy = doys, .packages=c("tidyverse","terra","data.table"), .combine = rbind) %dopar% {
      ...
      df <- fread(paste0("/glade/work/zhuonan/nclimgrid_scripts/US_GEOID/1df_latlong.csv"))
      oneday_df$long=df$x
      ...
      US_GEOID_1km <- rast(paste0("/glade/work/zhuonan/nclimgrid_scripts/US_GEOID/1US_GEOID_1km.tif"))
      ###############------------------------------------------------------------------------
...
for (yr in c(1957)) {
	summary_Climate(climvariable="tmax",year=yr)
}
