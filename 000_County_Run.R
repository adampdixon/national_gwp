...
if(identical(Test, TRUE)){
  county_data<-county_data%>%
    filter(GEOID %in% c(1051)) # 1075
}

# GEOIDs that have been used for development
# 1051, 12107, 25001, 31117, 40079, 47081
# 4012, 8113, 54087
# 5007, 4009, 5021, 4015
# 4011, 6113, 9001, 13103, 16067
# 31181, 13023, 13213, 20073, 42053

# Custom set of geoids here. Added after most counties have run.
#################################################################################
# Remove after running.
to_finish<-c(1001, 1043, 1045, 1047, 1049, 1051, 5135, 6003, 6109, 9003, 12017, 12019, 12029, 
             12041, 12043, 12051, 12055, 12071, 12085, 12093, 12107, 12111, 12121, 12123, 13189,
             13195, 16035, 17031, 18001, 18003, 18009, 18053, 18075, 20003, 20059, 20139, 22041,
             23017, 23019, 23021, 25001, 25003, 25011, 25015, 25023, 26019, 26027, 26029, 26047,
             26077, 26085, 26113, 26149, 26153, 26165, 28053, 28065, 28067, 28081, 28083, 28119,
             28125, 28133, 29043, 29059, 29067, 29091, 29093, 29105, 29145, 29149, 29179, 29225,
             29229, 30003, 30011, 30025, 30029, 30033, 30069, 31005, 31075, 31091, 31117, 31171,
             33003, 33007, 33009, 33019, 34005, 34009, 34011, 34015, 34019, 34021, 36003, 36007,
             36045, 36059, 36083, 36103, 36105, 39003, 39011, 39039, 39063, 39065, 39101, 39107,
             39125, 39137, 39149, 39159, 39161, 40077, 40079, 40089, 40099, 40121, 41005, 41011,
             41039, 41043, 41047, 41051, 41057, 44003, 44005, 46095, 46099, 46101, 46107, 46111,
             46115, 46125, 46135, 47009, 47011, 47013, 47015, 47017, 47023, 47027, 47031, 47041,
             47043, 47045, 47049, 47051, 47053, 47055, 47057, 47059, 47061, 47063, 47071, 47073,
             47081, 47083, 47087, 47093, 47095, 47097, 48155, 50011, 50015, 50023, 50025, 50027,
             51059, 51087, 51089, 51153, 51161, 51660, 51685, 51690, 51760, 51775, 53011, 53015,
             53027, 53029, 53031, 53045, 55011, 55091, 55125)


county_data<-county_data%>%
  filter(GEOID %in% to_finish) 
#################################################################################
...
cat(paste0("Starting county ", county_number, "\n"))
county_print_marker<-paste("county geoid, name, state, number is:", county_geoid, county_name, state_name, county_number)
