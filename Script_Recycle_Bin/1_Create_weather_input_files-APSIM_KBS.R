...
suppressMessages({
  ...
  if(clim_scenario_num==1) {
    ...
    APSIM_basic <- as_apsim_met(new_dat[,c("year","dayofyear","radn_MJm2.x",
  ...
  write_apsim_met(APSIM_basic, wrt.dir=apsim_path, filename="basic_wth_exp.met")
    ...
    APSIM_basic_fut <- as_apsim_met(new_dat_fut[,c("year","dayofyear","radn_MJm2.x",
                                                     "maxt_C.x","mint_C.x","rain_mm.x")],
  ...
  try(
    ...
    colnames(APSIM_basic) <- c("year","dayofyear","radn_MJm2",
