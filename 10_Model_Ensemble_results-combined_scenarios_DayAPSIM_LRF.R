...
scenario_means <- summary_output %>%
  ...
  summarize(mean_CottonYld_Mgha=round(mean(Cotton_Diff_Mgha,na.rm=T),5),
            ...
            sd_CottonYld_Mgha=round(mean(Cotton_Diff_Mgha,na.rm=T),5),
            sd_SorghumYld_Mgha=round(mean(Sorghum_Diff_Mgha,na.rm=T),5),
...
gGWPeb <- scenario_means[scenario_means$Climate_Scenario==clim_num,] %>%
  ...
  annotate("rect", xmin = 0.5, xmax = 5.5,
           ymin = min(summary_output[summary_output$Climate_Scenario==clim_num,"mean_GWP"]*1.05, na.rm=T),
           ymax = max(summary_output[summary_output$Climate_Scenario==clim_num,"mean_GWP"]*1.05, na.rm=T),
           alpha = 0, color= "grey") +
  ...
  annotate("rect", xmin = 8.5, xmax = 9.5,
           ...
           ymax = max(summary_output[summary_output$Climate_Scenario==clim_num,"mean_GWP"]*1.05, na.rm=T),
           alpha = 0, color= "grey") +
  annotate("rect", xmin = 9.5, xmax = 13.5,
           ymin = min(summary_output[summary_output$Climate_Scenario==clim_num,"mean_GWP"]*1.05, na.rm=T),
           ymax = max(summary_output[summary_output$Climate_Scenario==clim_num,"mean_GWP"]*1.05, na.rm=T),
           alpha = 0, color= "grey") +
  annotate("rect", xmin = 13.5, xmax = 19.5,
           ymin = min(summary_output[summary_output$Climate_Scenario==clim_num,"mean_GWP"]*1.05, na.rm=T),
           ymax = max(summary_output[summary_output$Climate_Scenario==clim_num,"mean_GWP"]*1.05, na.rm=T),
           alpha = 0, color= "grey") +
...
gN2Ochg_am <- summary_output[summary_output$Climate_Scenario==clim_num,] %>%
  ...
  annotate("rect", xmin = 0.5, xmax = 5.5,
           ymin = min(summary_output[summary_output$Climate_Scenario==clim_num,"mean_CO2e_N2O"]*1.05, na.rm=T),
           ymax = max(summary_output[summary_output$Climate_Scenario==clim_num,"mean_CO2e_N2O"]*1.05, na.rm=T),
           alpha = 0, color= "grey") +
  annotate("rect", xmin = 5.5, xmax = 6.5,
           ymin = min(summary_output[summary_output$Climate_Scenario==clim_num,"mean_CO2e_N2O"]*1.05, na.rm=T),
           ymax = max(summary_output[summary_output$Climate_Scenario==clim_num,"mean_CO2e_N2O"]*1.05, na.rm=T),
