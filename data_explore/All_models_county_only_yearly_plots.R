...
county_df<-get_county_models_df(crops_to_get = c('Maize'), GEOID = 20073 )
...
gfg_plot <- function(df, result){
  ...
  # TODO add if statement taking Millennial out if it's a non-millennial result

  df2<-select(df, year, model, climate_scenario, all_of(result))%>%
    mutate(model_climate = paste0(model, ' ', climate_scenario))
  
  p<-ggplot(df2) +
    geom_line(aes(x=year, y=get(result), group=model_climate, color = model_climate)) +
