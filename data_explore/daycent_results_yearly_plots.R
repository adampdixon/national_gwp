...
for (cl in c(1:2)){
  for (cr in c('Maize', 'Soybean', 'Wheat', 'Cotton', 'Rotation')) {
    ...
    for (s in 1:6){
      print(paste0('working on ', cr, ', practice scenario ', s, ' table, ', 'in climate scenario ', cl))
      
      # county_n<-0 # counter
      
      for (i in 1:length(r2)){
        files<-list.files(r1[i], full.names=T, recursive=T, pattern=paste0('Annual_results_compilation_', cl, '_', s, '_', cr, '_Daycent.csv'))
        # county_n<-county_n+1
        ...
        for (f in files){
          ...
          data<-fread(f)%>%
            ...
            select(GEOID, State, year, mgmt_scenario_num, paste0(select_var, 'Yld_Mgha'), SOC_Mgha, N2OEmissions_ghayr, CH4Emissions_ghayr, CO2resp_ghayr) #TODO add CO2
    ...
    gfg_plot <- function(df, result){
      ...
      # new df
      df2<-select(df, year, eval(paste0(result, '_Mean')), eval(paste0(result, '_SD')))%>%
        mutate(mean_value = .[[2]],
               sd_value = .[[3]])%>%
    ...
    crop_out<-file.path(output_figs, paste0('clim_', cl, "_", cr,  "_results.png"))
    
    ggsave(file = crop_out, plot=out, dpi=300, width = 19, height = 16)
