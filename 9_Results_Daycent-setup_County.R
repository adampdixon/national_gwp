...
# Day_exp_harvest <- read_csv(paste0(daycent_path,paste0("harvest_exp_",scenario_name2,".csv")),
#                             col_names = TRUE, show_col_types = F)
# Day_fut_harvest <- read_csv(paste0(daycent_path,paste0("harvest_fut_",scenario_name2,".csv")),
#                             col_names = TRUE, show_col_types = F)
...
# Day_exp_soiltavg <- read.fwf(file.path(daycent_path2, paste0("soiltavg_exp_", scenario_name2,".out")),
#                              widths=c(12,5,8,8,8,8,8,8,8,6), 
#                              col.names=c("time","dayofyear","layer1","layer2","layer3",
#                                          "layer4","layer5","layer6","layer7","layer8"),
#                              colClasses=c("numeric","numeric","numeric","numeric","numeric",
...
#   mutate(year=floor(time),
#          date=as.Date(dayofyear,origin=paste0(as.character(year),"-01-01"))-1)
# Day_fut_soiltavg <- read.fwf(file.path(daycent_path2,paste0("soiltavg_fut_",scenario_name2,".out")),
#                              widths=c(12,5,8,8,8,8,8,8,8,6), 
#                              col.names=c("time","dayofyear","layer1","layer2","layer3",
#                                          "layer4","layer5","layer6","layer7","layer8"),
#                              colClasses=c("numeric","numeric","numeric","numeric","numeric",
...
Day_base_vswc <- read.fwf(file.path(daycent_path2,paste0("vswc_base_",scenario_name2,".out")),
                          ...
                          colClasses=c("numeric","numeric","numeric","numeric","numeric",
                                       "numeric","numeric","numeric","numeric","numeric"))
# Day_exp_vswc <- read.fwf(file.path(daycent_path2,paste0("vswc_exp_",scenario_name2,".out")),
#                          widths=c(10,7,10,10,10,10,10,10,10,6), 
#                          col.names=c("time","dayofyear","layer1","layer2","layer3",
#                                      "layer4","layer5","layer6","layer7","layer8"),
#                          colClasses=c("numeric","numeric","numeric","numeric","numeric",
#                                       "numeric","numeric","numeric","numeric","numeric")) 
# Day_fut_vswc <- read.fwf(file.path(daycent_path2,paste0("vswc_fut_",scenario_name2,".out")),
#                          widths=c(10,7,10,10,10,10,10,10,10,6), 
#                          col.names=c("time","dayofyear","layer1","layer2","layer3",
#                                      "layer4","layer5","layer6","layer7","layer8"),
#                          colClasses=c("numeric","numeric","numeric","numeric","numeric",
...
# Day_exp_methane <- read.fwf(file.path(daycent_path2,paste0("methane_exp_",scenario_name2,".out")),
...
# 
#   Day_fut_methane <- read.fwf(paste0(daycent_path,paste0("methane_fut_",scenario_name2,".out")),
#                               widths=c(4,6,12,12,12,12,12,12,12,12,12,12,12,12,12,
#                                        12,12,12,12,12,12),
#                               col.names=c("year","DOY","aglivc","bglivcj","bglivcm",
#                                           "prev_mcprd1","prev_mcprd2","prev_mcprd3",
...
#   } else {
#     
#     Day_fut_methane <- read.fwf(file.path(daycent_path2,paste0("methane_fut_",scenario_name2,".out")),
#                                 widths=c(4,6,12,12,12,12,12,12,12,12,12,12,12,12,12,
#                                          12,12,12,12,12,12),
#                                 col.names=c("year","DOY","aglivc","bglivcj","bglivcm",
#                                             "prev_mcprd1","prev_mcprd2","prev_mcprd3",
...
Day_base_summary <- read.fwf(file.path(daycent_path2,paste0("summary_base_", scenario_name, ".out")),
                             ...
                             skip=1)
# Day_exp_summary <- read.fwf(file.path(daycent_path2,paste0("summary_exp_",scenario_name2,".out")),
#                             widths=c(10,5,9,9,9,13,13,13,13,13),
#                             col.names=c("time","dayofyear","tmax","tmin","ppt",
...
#   
# 
# Day_fut_summary <- read.fwf(file.path(daycent_path2,paste0("summary_fut_",scenario_name2,".out")),
#                             widths=c(10,5,9,9,9,13,13,13,13,13),
#                             col.names=c("time","dayofyear","tmax","tmin","ppt",
#                                         "N2O_gNhad","NOflux","CH4_oxid_gChad","NIT","CO2resp"),
#                             skip=1) %>%
...
# Day_exp_soiln <- read.fwf(file.path(daycent_path2,paste0("soiln_exp_",scenario_name2,".out")),
#                           widths=c(8,6,14,14,14,14,14,14,14,14,14,14,14,14,14,14),
#                            col.names=c("time","dayofyear","ammonium","NO3_ppm0","NO3_ppm1",
#                                        "NO3_ppm2","NO3_ppm3","NO3_ppm4","NO3_ppm5","NO3_ppm6",
#                                        "NO3_ppm7","NO3_ppm8","NO3_ppm9","NO3_ppm10",
...
# 
# 
# Day_fut_soiln <- read.fwf(file.path(daycent_path2,paste0("soiln_fut_",scenario_name2,".out")),
#                           widths=c(8,6,14,14,14,14,14,14,14,14,14,14,14,14,14,14),
#                           col.names=c("time","dayofyear","ammonium","NO3_ppm0","NO3_ppm1",
#                                       "NO3_ppm2","NO3_ppm3","NO3_ppm4","NO3_ppm5","NO3_ppm6",
#                                       "NO3_ppm7","NO3_ppm8","NO3_ppm9","NO3_ppm10",
...
# Day_exp_wfps <- read.fwf(file.path(daycent_path2,paste0("wfps_exp_",scenario_name2,".out")),
...
#          date=as.Date(dayofyear,origin=paste0(as.character(year),"-01-01"))-1)
# 
# Day_fut_wfps <- read.fwf(file.path(daycent_path2,paste0("wfps_fut_",scenario_name2,".out")),
#                          widths=c(8,5,9,9,9,9,9,9,9,9,9,9,9,9,9),
#                          col.names=c("time","dayofyear","wfps_layer1","wfps_layer2",
#                                      "wfps_layer3","wfps_layer4","wfps_layer5",
#                                      "wfps_layer6","wfps_layer7","wfps_layer8",
...
# lis_output_raw <- read.table(file.path(daycent_path2,paste0("sched_fut_",scenario_name2,".lis")),
...
DayC_Mgha <- lis_output[,c("time","somsc_gm2","year")] %>%  
  mutate(base=round(somsc_gm2/100,1)
  )
...
colnames(output_annual_data) <- c("year", paste0(crop, "Yld_Mgha"),"SOC_Mgha","model_name", 
                                  "scenario_name2","climate_scenario_num",
...
#                                  "model_name","scenario_name2","climate_scenario_num",
#                                  "mgmt_scenario_grp_num","mgmt_scenario_opt_num")
