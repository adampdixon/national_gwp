...
#    geom_abline(intercept=CYfit_APSIM[1], slope=CYfit_APSIM[2], color="orange") +
  ...
  NO35cm_fit_time <- lm(NO3_5cm/1000000 ~ date, data = NO3_this)
  NO35cm_fit_coef_time <- coef(NO35cm_fit_time)
  NO315cm_fit_time <- lm(NO3_15cm/1000000 ~ date, data = NO3_this)
  NO315cm_fit_coef_time <- coef(NO315cm_fit_time)
  NO335cm_fit_time <- lm(NO3_35cm/1000000 ~ date, data = NO3_this)
  ...
  N2O5cm_fit_time <- lm(N2O_5cm_ghaday/1000 ~ date, data = N2O_this)
  N2O5cm_fit_coef_time <- coef(N2O5cm_fit_time)
  N2O15cm_fit_time <- lm(N2O_15cm_ghaday/1000 ~ date, data = N2O_this)
  N2O15cm_fit_coef_time <- coef(N2O15cm_fit_time)
  N2O35cm_fit_time <- lm(N2O_35cm_ghaday/1000 ~ date, data = N2O_this)
  ...
  gN2O_expl_fut <- ggplot() +
    ...
    # geom_abline(intercept=SW15cm_fit_coef_time[1], slope=SW15cm_fit_coef_time[2], color=cbPalette9[2]) +
    # geom_abline(intercept=SW35cm_fit_coef_time[1], slope=SW35cm_fit_coef_time[2], color=cbPalette9[3]) +
  ...
  SoilT5cm_fit_time <- lm(SoilTemp_5cm_C ~ date, data = SoilT_this) # convert to fraction
  SoilT5cm_fit_coef_time <- coef(SoilT5cm_fit_time)
  SoilT15cm_fit_time <- lm(SoilTemp_15cm_C ~ date, data = SoilT_this) # convert to fraction
  SoilT15cm_fit_coef_time <- coef(SoilT15cm_fit_time)
  SoilT35cm_fit_time <- lm(SoilTemp_35cm_C ~ date, data = SoilT_this) # convert to fraction
  ...
  SoilT10cm_fit_time <- lm(SoilTemp_10cm_C ~ date, data = SoilT_this) # convert to fraction
  ...
  SOC10cm_fit_time <- lm(TotalSOC_10cm ~ year, data = SoilCN_this) # convert to fraction
  ...
  gBioC_expl_fut <- ggplot() +
    ...
    #           aes(x=date, y=VolH2O_35cm/100, color=cbPalette20[3]), linewidth=1) +
    # geom_line(data=APSIMM_V[APSIMM_V$year %in% obs_soc_years,],
    ...
    #           aes(x=date, y=SoilTemp_15cm_C, color="SoilT"), linewidth=1) +
    # geom_line(data=APSIMSoilCN_kgha[APSIMSoilCN_kgha$year %in% end_exp_period_year:end_fut_period_year,],
    #           aes(x=date, y=CtoBiom_10cm/10, color="CtoB"), linewidth=1) +
    # geom_line(data=APSIMSoilCN_kgha[APSIMSoilCN_kgha$year %in% end_exp_period_year:end_fut_period_year,],
    #           aes(x=date, y=CtoHum_10cm/10, color="CtoH"), linewidth=1) +
    ...
    #           aes(x=date, y=biomc_35cm, color=cbPalette20[7]), linewidth=1) +
    # geom_line(data=APSIMSoilCN_kgha[APSIMSoilCN_kgha$year %in% obs_soc_years,],
