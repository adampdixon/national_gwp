...
suppressMessages({
  ...
  MYfit_APSIM <- coef(lm(APSIM ~ year, 
                         data = MaizeYld_Mgha[MaizeYld_Mgha$year>experiment_end_year,]))
...
#    geom_abline(intercept=MYfit_APSIM[1], slope=MYfit_APSIM[2], color="orange") +
  ...
  SYfit_APSIM <- coef(lm(APSIM ~ year, 
                         data = SoyYld_Mgha[SoyYld_Mgha$year>experiment_end_year,]))
  ...
  WYfit_APSIM <- coef(lm(APSIM ~ year, 
                         data = WheatYld_Mgha[WheatYld_Mgha$year>experiment_end_year,]))
  ...
  NGfit_APSIM <- coef(lm(N2OEmissions_ghayr ~ year, 
                         data = APSIMGN_ann_gha[APSIMGN_ann_gha$year>experiment_end_year,]))
  ...
  gNGann <- N2O_ghayr_piv %>%
    ...
    ggtitle(bquote(.(site_name)~"Annual N"["2"]*"O Emissions"),
            paste0("Scenario: ",scenario_descriptor_full)) +
  ...
  # NGfit_APSIM <- coef(lm(N2O_gha ~ date, 
  #                        data = APSIMGN_cum_gha[APSIMGN_cum_gha$date>experiment_end_date,]))
  # NGxs <- c(experiment_end_year+1, end_fut_period_year)
  # NGys <- cbind(1, NGxs) %*% NGfit_APSIM
  ...
  gNGcum <- N2O_cum_piv %>%
    ...
    xlab("Year") +
    ylab(expression('N'[2]*'O Emissions (g ha ' ^-1*'yr ' ^-1*')')) +
    ggtitle(bquote(.(site_name)~"Daily Cumulative N"["2"]*"O Emissions"),
            paste0("Scenario: ",scenario_descriptor_full)) +
    #geom_segment(aes(x = NGxs[1], xend = NGxs[2], y = NGys[1], yend = NGys[2]), color=cbPalette9[8]) +
    scale_color_manual(labels=c("APSIM"),
                       values=cbPalette9[c(8)]) +
  ...
  ggsave(filename=paste0(results_path,"N2O_cum_comparison_fut_",clim_scenario_num,"_",mgmt_scenario_num,"_APSIM.jpg"),
         plot=gNGcum, width=9, height=6, dpi=300)
