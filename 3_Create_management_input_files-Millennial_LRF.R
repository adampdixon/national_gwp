...
print(paste0("Starting 3_Create_management_input_files-Millennial_",site_name,".R"))
...
# that the C input data is coming from Daycent, which is only to 20 cm, but
...
for(i in land_conversion_year:experiment_start_year-1) { 
  if (i == land_conversion_year) { # start with fresh file creation; all other write.table calls appends data
  ...
  } # end 1940 loop initialization
  if (i >= land_conversion_year+1 & i <= 1959) { # continuous corn
    base_inputdata[1:3] <- head(mill_inputdata[mill_inputdata$year==i,
                                               c("soil_T","soil_M","daily_soilC_gm2")],365)
    base_inputdata[,4] <- seq(as.Date(paste0(i,"-01-01")),
                              length.out=365,by="day")
    base_inputdata[,5] <- "sorghum"
    write.table(base_inputdata, file=paste0(mill_path,mill_baseinput_filename),
                col.names=FALSE, row.names=FALSE, sep="\t", 
                quote=FALSE, append=TRUE)
  } #end 1940-1959
  else   if (i >= 1960 & i < experiment_start_year) { # landman alternates 2 years: soybeans and corn, but C distribution is same
    base_inputdata[1:3] <- head(mill_inputdata[mill_inputdata$year==i,
                                               c("soil_T","soil_M","daily_soilC_gm2")],365)
    base_inputdata[,4] <- seq(as.Date(paste0(i,"-01-01")),
                              length.out=365,by="day")
    ...
    write.table(base_inputdata, file=paste0(mill_path,mill_baseinput_filename),
                col.names=FALSE, row.names=FALSE, sep="\t", 
                quote=FALSE, append=TRUE)
  } #end 1960-one year before start year
} #end 1940-2002
...
if(mgmt_scenario_grp!=7) {
  #if not continuous cotton
exp_inputdata <- mill_inputdata[mill_inputdata$year >= experiment_start_year &
                                  mill_inputdata$year <= end_exp_period_year,
                                c("date","soil_T","soil_M","daily_soilC_gm2")] %>% 
  mutate(year=year(date),
         crop=if_else(year >= experiment_start_year & year <= 2006 & (year %% 2)==1, "cotton",
              if_else(year >= experiment_start_year & year <= 2006 & (year %% 2)==0, "sorghum",
                      "cotton"))
  )
} else {
  # is continuous cotton
  exp_inputdata <- mill_inputdata[mill_inputdata$year >= experiment_start_year &
                                    mill_inputdata$year <= end_exp_period_year,
                                  c("date","soil_T","soil_M","daily_soilC_gm2")] %>% 
    mutate(year=year(date),
...
fut_inputdata <-mill_inputdata[mill_inputdata$year > end_exp_period_year,
                               c("date","soil_T","soil_M","daily_soilC_gm2")]  %>%
  mutate(year=year(date),
         crop=if_else((year %% 2)==0, "sorghum",
              "cotton")
  )
